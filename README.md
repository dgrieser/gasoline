...
Upgrading to per-user notification areas: `migrate` carries a user's old city selection over only when it says exactly what one area can express — a single selected city becomes that city's centre with its target's radius. A selection of several cities, or the old default of none (which meant *every* configured city), cannot become one area, so those users are left without one and named on stderr as `needs_area`; they receive nothing until they pick a city and radius. Narrowing them to one city silently would have been worse than a visible gap.
