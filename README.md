...
- the **city selection**: notifications cover only the update-target cities the user selected (multi-select in My Account → Notifications, stored in `user_notify_cities`). An empty selection means all configured cities, including ones added later. Deleting an update target also removes it from every user's selection. Two targets that name the same place — `Berlin` and `Berlin, Germany` are distinct target strings but one city — are treated as one city with two names: their stations are delivered once, against one baseline, and selecting either name selects it.
...
- A station inside two update targets' radii belongs to the nearest one, and `price_snapshots.city_name` records that owner. `suggest`, `check` and `notify` read it: it is what attributes each station to a city for the per-user notification filter and for the reported distance, and because a station has exactly one owner, overlapping targets can no longer deliver it twice. `gasoline stations --city` filters on the same column. The web viewer's own city filter is separate and still geometric — it is a browse filter the visitor drives, with its own radius selector.
