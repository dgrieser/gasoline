...
`gasoline suggest --persist` additionally stores the **full forecast grid** — every station and future hour it can score, with each station's own suggested windows flagged (`is_suggestion`) — plus a record of what the price check would decide right now, in three tables (`prediction_runs`, `price_predictions`, `price_check_decisions`, created automatically on first use). Run it on a timer (e.g. hourly, next to `gasoline update`); each run:

1. **Evaluates** stored predictions whose target hour has passed, filling in the actual price (the price in effect at the window midpoint) and the prediction error. Work is grouped by target window: every run re-predicts the same future hours, so one window ends up carrying one row per run inside its lead horizon, and all of them settle against the same recorded price — it is looked up once and written across the whole stack. A run keeps going until nothing is due, up to a quarter of a million rows per fuel, which is what lets it work off arrears instead of falling further behind them every hour. Whatever it could not reach is reported (see the summary line below) and picked up by the next run.
...
4. **Persists** the new grid; newer runs supersede older ones for the same target hour, older rows remain as learning history. `is_suggestion` flags, per station, the windows that station's own forecast would be suggested for — the same selection the notifier makes (cheapest hours of each local day, no two within two hours of each other, at most three a day), run over one station at a time. A run covers every station being fed while a suggestion is delivered per area, so what a run *prints* is only the globally cheapest handful; flagging that instead left the accuracy page with no suggested rows to show and measured the selection bias off whichever station happened to win. Per station is the set every per-area pick is drawn from — a window that is the cheapest in some area is necessarily the cheapest at its own station that day.
...
The normal suggestion output is unchanged; a one-line summary (`persist: stored N predictions, evaluated M, ..., pruned A/B by retention, C/D for E stations that left scope, evaluation caught up`) goes to stderr. Its last field is the evaluation backlog: `evaluation caught up` when nothing is waiting, otherwise how far behind the oldest unsettled window is (`evaluation 31.0h behind (oldest unsettled window …)`). It is also recorded per run as `evaluation_backlog_hours`, so the Statistics page shows whether it is growing — a rising backlog is the one thing that makes the accuracy page quietly stop at an old date, since it only ever sees evaluated rows. Pass `--quiet` (or `-q`) to suppress the suggestion output entirely and only store — useful for timer runs whose stdout nobody reads. One invocation persists a run per fuel over every fed station, so all of them accrue evaluation data for the bias learning; per-fuel failures are reported on stderr and via the exit code. The accrued evaluation data also feeds the bias learning and is surfaced in the web UI on the admin **Prediction accuracy** page (hamburger menu → Prediction accuracy), which compares each past predicted price with the actual price recorded for that window — raw rows, accuracy statistics (MAE, bias, RMSE, share within ±1/±2 ct, per-confidence breakdown), breakdowns by lead time and by hour of day, the alert outcomes described above, and a predicted-vs-actual graph.
...
| `suggest` | `persist` (0/1), `fuels`, `fuels_failed`, `stations`, `snapshots_scanned`; with `--persist` also `predictions_stored`, `decisions_stored`, `predictions_evaluated`, `evaluation_backlog_hours`, `outcomes_scored`, `stations_bias_corrected`, `pruned_predictions`, `pruned_decisions`, `unfed_stations`, `unfed_predictions`, `unfed_decisions` |
...
- **Prediction accuracy** (admins) — compare past predicted prices with the actual prices recorded for those windows, from the evaluated `price_predictions` data (see [Persistent predictions and learning](#persistent-predictions-and-learning-suggest---persist)). Filter by fuel, target date range, and confidence; view accuracy statistics (count, MAE, bias, RMSE, share within ±1/±2 ct, and a per-confidence breakdown), a predicted-vs-actual graph (timeline with an error band, or a predicted-vs-actual scatter), and the raw evaluated rows — the ones flagged as that station's suggested windows carry a star. A page that only ever shows evaluated rows has one silent failure mode, an evaluation that is behind: when the oldest window still waiting for an actual price is more than six hours old, a note under the filters says so and names it, so a range that stops days short of today explains itself instead of looking like a model that stopped predicting. The page renders before it reads anything: the filters, the empty tables and their spinners paint immediately and `?action=prediction_accuracy` fills them in, so a click opens a page rather than sitting on the previous one. That endpoint also picks which fuel to open on — the first with anything evaluated, preferring diesel — and the picker adopts its answer when the first payload lands.
